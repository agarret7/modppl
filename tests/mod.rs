...
use gen_reflex::dists::{Distribution, normal, uniform_2d, categorical};
...
fn test_normal() {
    ...
    let samples = (0..50000).map(|_| normal.random(&mut rng, &(0., 1.))).collect::<Vec<f32>>();
    let empirical_mu = stats::mean(samples.iter());
    let empirical_std = stats::standard_deviation(samples.iter(), 1);
    approx::assert_abs_diff_eq!(empirical_mu, 0., epsilon = 0.02);
    approx::assert_abs_diff_eq!(empirical_std, 1., epsilon = 0.02);
