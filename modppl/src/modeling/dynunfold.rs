use crate::Real;
use crate::{ArgDiff, DynGenFn, DynGenFnHandler, DynTrie, GenFn, ParticleSystem, Trace};
use rand::rngs::ThreadRng;

/// Combinator struct for kernels that use the `DynGenFnHandler` DSL (`sample_at` and `trace_at`).
...
pub struct DynUnfold<State> {
    /// A stochastic kernel that takes in a mutable reference to a `DynGenFnHandler<A,T>` and some `State`, effectfully mutates it, and produces a new `State`.
    pub kernel: DynGenFn<(i64, State), State>,
}

impl<State> DynUnfold<State> {
    /// Dynamically construct a `DynUnfold` from a kernel function at run-time.
...
impl<State: Clone> GenFn<(i64, State), Vec<DynTrie>, Vec<State>> for DynUnfold<State> {
    fn simulate(
    ...
    ) -> Trace<(i64, State), Vec<DynTrie>, Vec<State>> {
        let (final_t, mut state) = final_t_and_args;
        assert!(final_t >= 1);
        let mut vec_trace = Trace {
            args: (final_t, state.clone()),
            data: vec![],
            retv: Some(vec![]),
            logjp: 0.,
        };
        for t in 0..final_t {
            ...
            let DynGenFnHandler::Simulate { prng: _, trace } = g else {
                unreachable!()
            };
    ...
    ) -> (Trace<(i64, State), Vec<DynTrie>, Vec<State>>, Real) {
        let (final_t, mut state) = final_t_and_args;
        assert!(final_t >= 1);
        let mut vec_trace = Trace {
            args: (final_t, state.clone()),
    ...
    fn update(
            ...
            ArgDiff::Extend => {
                for (t, constraints) in vec_constraints.into_iter().enumerate() {
                    let mut g = DynGenFnHandler::Generate {
                        prng: &mut ThreadRng::default(),
                        trace: Trace {
                            args: (prev_t + (t as i64), state.clone()),
                            data: DynTrie::new(),
                            retv: None,
                            logjp: 0.,
                        },
        ...
        (
            vec_trace,
            (prev_t..final_t).map(|_| DynTrie::new()).collect::<_>(),
            update_weight,
        )
    }
}
