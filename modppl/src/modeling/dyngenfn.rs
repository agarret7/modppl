...
impl<A,T> DynGenFnHandler<'_,A,T> {
    ...
    /// If updating, move the visitor complement in `trace.data.schema()` into the discard, subtracting the corresponding weight.
    /// If regenerating, just move the visitor complement in `trace.data.schema()` into the discard.
