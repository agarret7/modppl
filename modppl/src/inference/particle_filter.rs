...
use crate::{categorical, logsumexp, ArgDiff, Distribution, GenFn, Trace};
...
pub struct ParticleSystem<Args: Clone, Data: Clone, Ret: Clone, F: GenFn<(i64, Args), Data, Ret>> {
    ...
    log_normalized_weights: Vec<Real>,
    two_times_log_normalized_weights: Vec<Real>,
    normalized_weights: Vec<Real>,

    parents: Vec<usize>,
    rng: ThreadRng,

    log_ml_estimate: Real,
}

impl<Args: Clone, Data: Clone, Ret: Clone, F: GenFn<(i64, Args), Data, Ret>>
    ParticleSystem<Args, Data, Ret, F>
{
    fn normalize_weights(&mut self) -> Real {
    ...
    fn multinomial_resampling(&mut self) {
        for i in 0..self.num_particles {
            self.parents[i] =
                categorical.random(&mut self.rng, self.normalized_weights.clone()) as usize;
        }
    }
    ...
    pub fn step(self, constraints: Data) -> Self {
        ...
        for (i, trace) in self.traces.into_iter().enumerate() {
            ...
            let (new_trace, _, log_weight) =
                self.model
                    .update(trace, new_args, ArgDiff::Extend, constraints.clone());
    ...
    pub fn effective_sample_size(&self) -> Real {
    ...
    pub fn resample(&mut self) -> Real {
        let log_total_weight = self.normalize_weights();
        self.log_ml_estimate += log_total_weight - (self.num_particles as Real).ln();
    ...
    pub fn log_marginal_likelihood_estimate(&self) -> Real {
        self.log_ml_estimate + logsumexp(&self.log_weights) - (self.num_particles as Real).ln()
    }
}
