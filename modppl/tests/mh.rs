...
use pointed_model::{DriftProposal, PointedModel};

mod dyngenfns;
use dyngenfns::{
    add_or_remove_param_proposal, hierarchical_drift_proposal, hierarchical_model, read_coeffs,
};
use dyngenfns::{pointed_2d_drift_proposal, pointed_2d_model};
...
fn test_metropolis_hastings_handcoded() -> std::io::Result<()> {
    ...
    let proposal = DriftProposal {
        drift_cov: dmatrix![0.25, 0.; 0., 0.25],
    };
    let bounds = Bounds {
        xmin: -5.,
        xmax: 5.,
    ...
    for iter in 0..NUM_ITERS {
        dbg!(iter);
        let (new_trace, accepted) = modppl::mh(&model, trace, &proposal, ());
        dbg!(accepted);
        trace = new_trace;
...
pub fn test_metropolis_hastings_dyngenfn() -> std::io::Result<()> {
    ...
    let mut trace = pointed_2d_model
        .generate((bounds, dmatrix![1., -3./5.; -3./5., 2.]), observations)
        .0;
    for iter in 0..NUM_ITERS {
        dbg!(iter);
        let (new_trace, accepted) = mh(
            ...
            dmatrix![0.25, 0.; 0., 0.25],
        );
        dbg!(accepted);
        trace = new_trace;
        let data = trace.data.read::<DVector<Real>>("latent");
        let json = format!("[{},{}]", data[0], data[1]);
        write(format!("../data/mh_trace_{}.json", iter), json)?;
    }
...
pub fn test_metropolis_hastings_hierarchical() -> std::io::Result<()> {
    ...
    ys.into_iter().enumerate().for_each(|(i, y)| {
        observations.observe(&format!("(y, {})", i), Arc::new(y));
    });
