...
use modppl::{categorical, importance_sampling, normal, Distribution, Real, Trace, Trie};

mod pointed_model;
use pointed_model::types_2d::Bounds;
use pointed_model::{PointedModel, PointedTrace};

mod dyngenfns;
use dyngenfns::{hierarchical_model, line_model};
...
fn test_importance_handcoded() -> std::io::Result<()> {
    ...
    let model = PointedModel {
        obs_cov: dmatrix![1., -3./5.; -3./5., 2.],
    };
    let bounds = Bounds {
        xmin: -5.,
        xmax: 5.,
    ...
    let (traces, log_normalized_weights, log_ml_estimate) =
        modppl::importance_sampling(&model, bounds, constraints, NUM_SAMPLES);

    dbg!(log_ml_estimate);

    let data = traces
        .iter()
        .map(|tr| tr.data.0.as_ref().unwrap().data.as_vec().to_vec())
        .collect::<Vec<Vec<Real>>>();
    let json = serde_json::to_string(&data)?;
    write("../data/initial_traces.json", json)?;

    let probs = log_normalized_weights
        .iter()
        .map(|w| w.exp())
        .collect::<Vec<Real>>();
...
pub fn test_importance_dyngenfn() {
    ...
    xs.iter().enumerate().for_each(|(i, x)| {
        observations.observe(
            ...
            Arc::new(0.5 * x - 1. + normal.random(&mut rng, (0., 0.1)))
                as Arc<dyn Any + Send + Sync>,
        );
    });
    let (traces, log_normalized_weights, lml_estimate) =
        importance_sampling(&line_model, xs, observations, NUM_SAMPLES);
    ...
    for i in 0..20 {
        println!("Trace {}", i);
        println!("slope = {}", &traces[i].data.read::<Real>("slope"));
        println!("intercept = {}", &traces[i].data.read::<Real>("intercept"));
    }
    dbg!(lml_estimate);
}
...
pub fn test_importance_hierarchical() -> std::io::Result<()> {
    ...
    let ys = xs
        .iter()
        .map(|x| a + b * x + c * x * x + normal.random(&mut rng, (0., 0.1)))
        .collect::<Vec<Real>>();
    write(
        "../data/hierarchical_data.json",
        format!("[{:?}, {:?}]", xs, ys),
    )?;
    ys.into_iter().enumerate().for_each(|(i, y)| {
        observations.observe(
            &format!("(y, {})", i),
            Arc::new(y) as Arc<dyn Any + Send + Sync>,
        );
    });
    ...
    let probs = log_normalized_weights
        ...
        .map(|w| w.exp())
        .collect::<Vec<Real>>();
    let traces = (0..(NUM_SAMPLES as Real).sqrt().trunc() as u32)
        .map(|_| categorical.random(&mut rng, probs.clone()))
        .map(|idx| &traces[idx as usize])
        .collect::<Vec<&Trace<_, _, _>>>();
    ...
    for i in 0..20 {
        ...
        let a = traces[i].data.read::<Real>("coeffs / a");
        let b = traces[i].data.read::<Real>("coeffs / b");
        let coeffs = if !*is_linear {
            let c = traces[i].data.read::<Real>("coeffs / c");
            vec![a, b, c]
        } else {
            vec![a, b]
        };
    ...
    write(
        "../data/hierarchical_model_is.json",
        format!("{:?}", all_coeffs),
    )?;
