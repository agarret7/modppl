...
use modppl::{ParticleSystem, Real};
...
fn test_hmm_forward_algorithm() {
    ...
    // z = [0, 0]
    true_marginal_likelihood += prior[0]
        * emission_dists[(obs[0] as usize, 0)]
        * transition_dists[(0, 0)]
        * emission_dists[(obs[1] as usize, 0)];
    // z = [0, 1]
    true_marginal_likelihood += prior[0]
        * emission_dists[(obs[0] as usize, 0)]
        * transition_dists[(1, 0)]
        * emission_dists[(obs[1] as usize, 1)];
    // z = [1, 0]
    true_marginal_likelihood += prior[1]
        * emission_dists[(obs[0] as usize, 1)]
        * transition_dists[(0, 1)]
        * emission_dists[(obs[1] as usize, 0)];
    // z = [1, 1]
    true_marginal_likelihood += prior[1]
        * emission_dists[(obs[0] as usize, 1)]
        * transition_dists[(1, 1)]
        * emission_dists[(obs[1] as usize, 1)];

    let empirical_marginal_likelihood =
        hmm::hmm_forward_alg(prior, emission_dists, transition_dists, &obs);
    approx::assert_abs_diff_eq!(
        empirical_marginal_likelihood,
        true_marginal_likelihood,
...
fn test_particle_filter() -> std::io::Result<()> {
    ...
    .transpose();
    let params = hmm::HMMParams::new(
        prior.clone(),
        emission_matrix.clone(),
