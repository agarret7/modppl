use modppl::real::consts::PI;
use modppl::{inference::ParticleSystem, normal, u01, Distribution, DynTrie, Real};
...
fn simulate_loop(rng: &mut ThreadRng, bounds: &Bounds, timesteps: i64) -> Vec<DynTrie> {
    ...
    let xrange = (bounds.xmax - bounds.xmin) as Real;
    let yrange = (bounds.ymax - bounds.ymin) as Real;
    let center = dvector![xrange / 2. + bounds.xmin, yrange / 2. + bounds.ymin];
    let radius = Real::max(bounds.xmax - bounds.xmin, bounds.ymax - bounds.ymin) / 5.;

    let mut observations = vec![];
    let perturb_means = (0..timesteps)
        .filter(|_| u01(rng) < 0.3)
        .collect::<Vec<_>>();
    for t in 0..timesteps {
        let mut deformation = 0.;
        for perturb_t in &perturb_means {
            deformation += normal
                .logpdf(&(t as Real), (perturb_t.clone() as Real, 1.))
                .exp()
        }
        let r = radius + deformation;
        let t = 2. * PI * (t as Real) / timesteps as Real;
        let obs = dvector![
...
fn test_smc() -> std::io::Result<()> {
    ...
    let obs = data
        .iter()
        .map(|t| t.read::<Point>("obs"))
        .collect::<Vec<Point>>();
    let obs_strs = obs
        .iter()
        .map(|obs| format!("[{},{}]", obs[0], obs[1]))
        .collect::<Vec<String>>();
    ...
    let states = filter
        .traces
        .iter()
        .map(|vtr| vtr.retv.as_ref().unwrap().last().unwrap().clone())
        .collect::<Vec<_>>();
    let state_strs = states
        .iter()
        .map(|latent| format!("[{},{}]", latent[0], latent[1]))
        .collect::<Vec<String>>();
    ...
    for (t, constraints) in data_it.enumerate() {
        ...
        let json = "[".to_owned() + &state_strs.join(", ") + "]";
        write(
            format!("../data/smc_traces_before_resample_{}.json", t + 1),
            json,
        )?;
        filter.resample();
        let states = filter
