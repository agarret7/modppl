...
fn _pointed_2d_drift_proposal(state: &mut TrieFnState<(Weak<Trace<(Bounds, DMatrix<f64>),Trie<(Rc<dyn Any>,f64)>,Point>>, DMatrix<f64>),()>,
    ...
    let latent = trace.get_data().get_leaf_node("latent").unwrap().clone().0.downcast::<DVector<f64>>().ok().unwrap();
    let new_latent = state.sample_at(&mut mvnormal, (latent.as_ref().clone(), noise), "latent");
}
...
pub fn test_mcmc() -> std::io::Result<()>{
    ...
    for iter in 0..NUM_ITERS {
        ...
        dbg!(accepted);
        trace = new_trace;
        let data = trace.get_data().get_leaf_node("latent").unwrap().0.clone().downcast::<DVector<f64>>().ok().unwrap();
