...
// searching the address of a w_observeed value should yield the leaf trie
#[test]
pub fn test_search_w_observed_value() {
    ...
    let found = root.search("test").expect("w_observeed value not found.");
    assert_eq!(*found, leaf);

    root.w_observe("test/deep/nested", 5, -1.2);
    let found = root.search("test/deep/nested").expect("w_observeed value not found.");
